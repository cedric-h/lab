(function()
{
  ...
  EntityPool.prototype.addComponent = function(id, component) {
    ...
    this.emitter.emit(component + "Create", id);
    ...
    if (typeof existingValue === "undefined") {
      ...
      for (var i = 0; i < this.componentToSearches[component].length; i++) {
        ...
        if (objectHasProperties(this.searchToComponents[search], this.entities[id])) {
          ...
          let previousIndex = this.searchResults[search].indexOf(id);
          if(previousIndex !== -1)
            this.searchResults[search].splice(previousIndex, 1);
  ...
  {
    define(['./object-pool.js', '../EventEmitter.js'], (objectpoolfile, eventemitterfile) =>
      {
        ObjectPool = objectpoolfile;
        EventEmitter = eventemitterfile;
        setImmediate = callback => Promise.resolve().then(callback);
        return EntityPool;
      });
  }
})();